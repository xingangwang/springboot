...
public class CheckMobileUtil {
    ...
    private static Logger _LOGGER = LoggerFactory.getLogger(CheckMobileUtil.class);
    ...
    public static boolean isMobileClient() {
        ...
        try{
            ...
            isFromMobile=check(request);
            //判断是否为移动端访问
            if(isFromMobile){
                _LOGGER.info("当前请求客户端判断为移动客户端，USER-AGENT:{}", request.getHeader( "USER-AGENT" ));
                isFromMobile =true;
            }
        }catch(Exception e){
            _LOGGER.error("an error occur...", e);
        }
